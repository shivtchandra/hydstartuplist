import { inferRoleType, inferExperienceLevel, parseYearsRequirement } from './job-facets.js';
import { canonicalJobUrl, collapseDuplicatePostings, deduplicateJobs, validDate } from './job-lifecycle.js';
import { isInvalidJobListing } from './job-content.js';

/** Short years label baked onto rows so cards stay honest after description is stripped. */
function experienceFromJob(j) {
  if (j.experience && typeof j.experience === 'string' && j.experience.trim()) return j.experience.trim();
  if (Number.isFinite(Number(j.startYear))) {
    const start = Number(j.startYear);
    const end = Number.isFinite(Number(j.endYear)) ? Number(j.endYear) : null;
  ...
  const parsed = parseYearsRequirement(`${j.title || ''} ${j.description || ''}`);
  if (!parsed) return null;
  const { lo, hi } = parsed;
  if (lo === 0 && hi <= 1) return 'Fresher (0–1y)';
...
export const FILTER_KEYS = ['q','role','level','area','work','salary','type','when','sort','company','bounds','direct','hyd'];

const HYD_AREA_SET = new Set([
  'Financial District','Nanakramguda','HITEC City','Gachibowli','Madhapur',
  'Jubilee Hills','Banjara Hills','Secunderabad','Kukatpally','Kondapur',
  'Begumpet','Raidurg','Shamshabad','Kompally','Uppal','Ameerpet',
  'Somajiguda','Manikonda','Narsingi','Miyapur','Punjagutta','Balanagar',
  'Hussainialam','Hyderabad',
]);
export function isHydArea(area) { return HYD_AREA_SET.has(area); }
...
export function cleanArea(value) {
  ...
  const found = areas.find(a=>text.includes(a.toLowerCase()));
  if (found) return found;
  if (/hitech|hi-tec/.test(text)) return 'HITEC City';
  if (/remote|anywhere|work from home|telehealth/i.test(text)) return 'Remote';
  if (/hyderabad|telangana|secunderabad/i.test(text)) return 'Hyderabad';
  return value || 'Hyderabad';
}
export function prepareJobs(jobs, companies = []) {
  const validJobs = (jobs || []).filter(j => !isInvalidJobListing(j));
  ...
  return collapseDuplicatePostings(deduplicateJobs(validJobs).map(j=>{
    const company=byId.get(j.startupId) || byName.get(searchText(j.company));
    const isDirect = j.source === 'ats' || j.source === 'careers' || !!j.atsProvider;
    const level = inferExperienceLevel(j.title, j.description, j) || 'unknown';
    const experience = experienceFromJob(j);
    return {...j, area:cleanArea(j.location || j.area || company?.area), role:inferRoleType(j.title) || 'Other',
      level,
      experience: experience || j.experience || null,
      levelInferred:!j.experienceLevel && !j.experience && j.startYear == null, work:workArrangement(j),
...
export function matchesJob(j, f, now=Date.now()) {
  ...
  if (f.direct === 'yes' && !j.isDirect) return false;
  ...
  // Sponsored / exclusive boosts may be outside Hyd (e.g. Vijayawada) — still show when pinned.
  if (f.hyd === 'yes' && !isHydArea(j.area) && !j.sponsored) return false;
...
export function filterJobs(jobs,filters,now=Date.now()) {
  return jobs.filter(j=>matchesJob(j,filters,now)).sort((a,b)=>{
    if (filters.sort === 'company') return a.company.localeCompare(b.company);
    const pin = Number(!!b.sponsored) - Number(!!a.sponsored);
    if (pin) return pin;
    return jobSortTime(b) - jobSortTime(a) || String(a.id).localeCompare(String(b.id));
  });
}
...
export function jobSummary({ description, ...j }) {
  // Keep blurb on exclusive/sponsored pins so the jobs board can show apply details.
  if (j.sponsored || j.exclusive) return { ...j, description: description || null };
  return j;
}
