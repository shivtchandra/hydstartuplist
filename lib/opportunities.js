...
import { canonicalJobUrl, deduplicateJobs, validDate } from './job-lifecycle.js';
...
export function prepareJobs(jobs, companies = []) {
  ...
  return deduplicateJobs(jobs).map(j=>{
    ...
    return {...j, area:cleanArea(j.area || company?.area || j.location), role:inferRoleType(j.title) || 'Other',
      ...
      postedAt:validDate(j.source==='ats'?j.sourcePostedAt:j.sourcePostedAt||j.postedAt),
      sourcePostedAt:validDate(j.source==='ats'?j.sourcePostedAt:j.sourcePostedAt||j.postedAt),
      applyUrl:canonicalJobUrl(j.url), category:j.category || 'other'};
  });
}
...
export function filterJobs(jobs,filters,now=Date.now()) {
  return jobs.filter(j=>matchesJob(j,filters,now)).sort((a,b)=>filters.sort==='company' ? a.company.localeCompare(b.company) :
    (Date.parse(b.sourcePostedAt || b.firstSeenAt || '')||0)-(Date.parse(a.sourcePostedAt || a.firstSeenAt || '')||0) || String(a.id).localeCompare(String(b.id)));
}
