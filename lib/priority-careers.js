...
const ROLE_RE =
  /(?:Engineer|Developer|Manager|Analyst|Lead|Specialist|Designer|Consultant|Architect|Intern|Director|Scientist|SRE|QA|Product|Sales|Marketing|Finance|Operations|Recruiter|Accountant|Officer|Executive|Coordinator|Technician|Welding|Avionics|Composites|Cryogenic|Dietitian|Dietician|Nutritionist|Nutrition|Coach|Counselor|HR\b)/i;
const MAX_ROLES = 40;
...
function titleFromSlug(slug) {
  // Boards like freshteam nest an opaque job id above the readable slug: /jobs/{id}/{slug}
  const readable = String(slug || "").split("/").filter(Boolean).pop() || "";
  return readable
    .replace(/\.[a-z]+$/i, "")
    .replace(/[-_]+/g, " ")
...
const NON_JOB_URL_RE =
  /\/(?:speciality|specialties|specialty|treatments?|procedures?|doctors?|departments?|services?|packages?|health-package|blogs?|news|articles?|patient-care|about-us|contact-us|facilities|infrastructure|conditions|clinics?|children-care-hospital|care-hospital)\b/i;

const NON_JOB_TITLE_RE =
  /\b(?:treatment|treatments|procedure|procedures|speciality|specialties|specialty|department|doctor profile|patient care|book appointment|consult doctor|health checkup|opd|emergency service|our doctors|find a doctor|health tips|medical service|hospital in\b|best hospital|care hospital)\b/i;
...
function dedupeRoles(roles) {
  ...
  for (const r of roles) {
    ...
    if (
      /cookie|privacy|view open|submit resume|learn more|read more|apply now|see all|view all|glossary|helpdesk|hr suite|hr templates|hr analytics|chro|cfo finance/i.test(
        title
      )
    )
      continue;
    if (NON_JOB_URL_RE.test(String(url)) || NON_JOB_TITLE_RE.test(title)) continue;
...
async function fetchLever(slug, boardUrl) {
  ...
  const hyd = data.filter((j) => {
    const loc = j.categories?.location || j.location || "";
    return HYD_TG_LOC_RE.test(loc) || /\b(remote|india|anywhere|telehealth)\b/i.test(loc);
  });
...
async function fetchGreenhouse(slug, boardUrl) {
  ...
  const hyd = data.jobs.filter((j) => {
    const loc = j.location?.name || "";
    return HYD_TG_LOC_RE.test(loc) || /\b(remote|india|anywhere|telehealth)\b/i.test(loc);
  });
  const picked = hyd.length ? hyd : data.jobs;
  const roles = picked.slice(0, MAX_ROLES).map((j) => ({
    title: j.title,
    url: j.absolute_url || boardUrl,
    description: sanitizeJobHtml(j.content || "") || null,
    postedAt: isoDate(j.updated_at || null),
  }));
  return pack({ name: slug }, "greenhouse", boardUrl || `https://boards.greenhouse.io/${slug}`, roles);
}

async function fetchAshby(slug, boardUrl) {
  const data = await getJson(`https://api.ashbyhq.com/posting-api/job-board/${slug}?includeCompensation=true`);
  if (!Array.isArray(data?.jobs) || !data.jobs.length) return null;
  const hyd = data.jobs.filter((j) => {
    const loc = Array.isArray(j.locations) ? j.locations.map((l) => l.name || "").join(" ") : j.location || "";
    return HYD_TG_LOC_RE.test(loc) || /\b(remote|india|anywhere|telehealth)\b/i.test(loc);
  });
  const picked = hyd.length ? hyd : data.jobs;
  const roles = picked.slice(0, MAX_ROLES).map((j) => ({
    title: j.title,
    url: j.jobUrl || j.applyUrl || boardUrl,
    description: sanitizeJobHtml(j.descriptionHtml || j.description || "") || null,
    postedAt: isoDate(j.publishedAt || j.createdAt || null),
  }));
  return pack({ name: slug }, "ashby", boardUrl || `https://jobs.ashbyhq.com/${slug}`, roles);
}
...
function extractTitleFromAnchor(innerHtml, href) {
  // Check headings or prominent paragraphs first
  const headMatches = [...innerHtml.matchAll(/<(?:h[1-6]|p|div|span)[^>]*>([^<]{3,80})<\/(?:h[1-6]|p|div|span)>/gi)];
  for (const hm of headMatches) {
    const candidate = hm[1].replace(/&amp;/g, '&').replace(/&nbsp;/g, ' ').replace(/&#39;/g, "'").trim();
    if (/^apply(\s+now)?$/i.test(candidate)) continue;
    if (
      ROLE_RE.test(candidate) &&
      candidate.length <= 60 &&
      !/^(remote|full[\s-]*time|part[\s-]*time|india|bangalore|hyderabad|onsite|hybrid)$/i.test(candidate) &&
      !NON_JOB_TITLE_RE.test(candidate)
    ) {
      return candidate;
    }
  }

  // Check URL slug if present
  const slug = (href || '').split('/').filter(Boolean).pop();
  if (slug && slug.length >= 4) {
    const slugTitle = titleFromSlug(slug);
    if (ROLE_RE.test(slugTitle) && slugTitle.length <= 60 && !NON_JOB_TITLE_RE.test(slugTitle)) {
      return slugTitle;
    }
  }

  const raw = cleanTitle(innerHtml);
  return raw.replace(/^apply\s+/i, '').trim();
}
...
function htmlLinkRoles(html, careersUrl) {
  ...
  for (const m of html.matchAll(/href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    if (/^(javascript:|mailto:)/i.test(m[1]) || NON_JOB_URL_RE.test(url)) continue;

    const title = extractTitleFromAnchor(m[2], m[1]);
    if (!ROLE_RE.test(title) || NON_JOB_TITLE_RE.test(title) || title.length > 80) continue;
  ...
  for (const m of html.matchAll(/href=["']([^"']*\/jobs?\/[^"'?#]+)["']/gi)) {
    ...
    if (NON_JOB_URL_RE.test(url)) continue;
    const slug = m[1].split(/\/jobs?\//i).pop() || "";
    const title = titleFromSlug(slug);
    if (title.length >= 4 && !NON_JOB_TITLE_RE.test(title)) roles.push({ title, url });
  }
  // Heading-ish job titles near apply links
  for (const m of html.matchAll(/<(?:h[1-4]|div|span|li)[^>]*>([^<]{6,90})<\/(?:h[1-4]|div|span|li)>/gi)) {
    const title = cleanTitle(m[1]);
    if (ROLE_RE.test(title) && !NON_JOB_TITLE_RE.test(title)) roles.push({ title, url: careersUrl });
  }
  return roles;
}
...
export async function scrapePriorityEmployer(entry) {
  if (entry.atsProvider === "greenhouse" && entry.atsSlug) {
    const hit = await fetchGreenhouse(entry.atsSlug, entry.url);
    if (hit) return hit;
  }
  if (entry.atsProvider === "ashby" && entry.atsSlug) {
    const hit = await fetchAshby(entry.atsSlug, entry.url);
    if (hit) return hit;
  }
