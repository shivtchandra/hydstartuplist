...
// Priority list is tiny (daily) — use Firecrawl when HTML fails if a key is present.
// Set PRIORITY_FIRECRAWL=0 to disable. Global FIRECRAWL_ENABLED is for check-hiring only.
...
function dedupeRoles(roles) {
  ...
  for (const r of roles) {
    ...
    if (/cookie|privacy|view open|submit resume|learn more|read more|apply now|see all|view all/i.test(title)) continue;
...
function schemaOrgRoles(html, careersUrl) {
  const matches = [...html.matchAll(/"@type"\s*:\s*"JobPosting"[\s\S]{0,800}?"title"\s*:\s*"([^"]+)"/gi)];
