...
const NON_JOB_URL_RE =
  /\/(?:speciality|specialties|treatments?|procedures?|doctors?|departments?|services?|packages?|health-package|blogs?|news|articles?|patient-care|about-us|contact-us|facilities|infrastructure|conditions|clinics?)\b/i;

const NON_JOB_TITLE_RE =
  /\b(?:treatment|treatments|procedure|procedures|speciality|specialties|department|doctor profile|patient care|book appointment|consult doctor|health checkup|opd|emergency service|our doctors|find a doctor|health tips|medical service)\b/i;
...
function htmlLinkRoles(html, careersUrl) {
  ...
  for (const m of html.matchAll(/href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    if (!ROLE_RE.test(title) || NON_JOB_TITLE_RE.test(title)) continue;
    let url;
    try {
