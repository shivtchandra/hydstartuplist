...
const getPersistedRuntimeJobs = unstable_cache(
  async () => {
    ...
    return { jobs, sourceStale: jobs.sourceStale === true };
  },
  ["public-jobs-v2"],
...
export const getAllJobs = cache(async () => {
  ...
  if (isNextProductionBuild()) {
    ...
    const cached = await getPersistedRuntimeJobs();
    jobs = cached.jobs;
    Object.defineProperty(jobs, "sourceStale", { value: cached.sourceStale === true });
  }
  jobsMemCache = jobs;
