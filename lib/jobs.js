...
async function withSector(jobs) {
  try {
    ...
    return jobs.map((j) => {
      if (j.sector) return { ...j, sector: normalizeSector(j.sector) };
      const hit = j.company ? matcher.resolve(j.company) : null;
      const sector = normalizeSector(hit?.startup?.sector || null);
      return { ...j, sector };
    });
  } catch (err) {
...
export async function getAllJobs() {
  ...
  if (!db) {
    const categorized = picks.map((j) => ({ ...j, category: categorize(j, gccs) }));
    return withBoost(await withSector(categorized));
  }
  ...
  try {
    ...
    const categorized = merged.map((j) => ({ ...j, category: categorize(j, gccs) }));
    return withBoost(await withSector(categorized));
  } catch (err) {
