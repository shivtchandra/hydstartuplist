...
import { getJobBoostsAsync, jobBoostFor, jobIsBoosted } from "./placements.js";
import { buildCompanyMatcher, jobMatchesStartup } from "./jobs-match.js";
import { normalizeSector, inferExperienceLevel, dropOtherMetroRows, roleFacetKey } from "./job-facets.js";
import { cleanScrapedTitle, isSpamOrConsultancyJob, isInvalidJobListing } from "./job-content.js";
...
async function withBoost(jobs) {
  ...
  return jobs
    .map((j) => {
      const boost = jobBoostFor(j, boosts);
      return {
        ...j,
        sponsored: !!boost || jobIsBoosted(j, boosts),
        sponsoredLabel: boost?.label || (boost ? "Exclusive" : null),
      };
    })
    .sort((a, b) => {
      const pin = Number(!!b.sponsored) - Number(!!a.sponsored);
      if (pin) return pin;
      const score = (j) => {
...
function mergeJobFeeds(atsJobs, adzunaJobs, careerJobs, priorityJobs = [], boardNames = [], empleosJobs = [], healthcareJobs = []) {
  ...
  const push = (job, rank) => {
    if (!job || isInvalidJobListing(job)) return;
  ...
  for (const j of healthcareJobs) push(j, 2);
  for (const j of empleosJobs) push(j, 3);
  for (const j of careerJobs) push(j, 4);
  for (const j of adzunaFiltered) push(j, 5);
  return out;
}
...
async function loadAllJobs() {
  ...
  try {
    const [atsSnap, adzunaSnap, prioritySnap, empleosSnap, healthcareSnap] = await Promise.all([
      readFeed("ats_latest"),
      readFeed("adzuna_latest"),
      readFeed("priority_careers_latest"),
      readFeed("empleos_latest"),
      readFeed("healthcare_latest"),
    ]);
    const atsJobs = stampAtsBoardMeta(atsSnap?.exists ? atsSnap.data().jobs || [] : []).filter(j => !isInvalidJobListing(j));
    ...
    const priorityJobs = (prioritySnap?.exists ? prioritySnap.data().jobs || [] : []).filter(j => !isInvalidJobListing(j));
    const empleosJobs = (empleosSnap?.exists ? empleosSnap.data().jobs || [] : []).filter(j => !isInvalidJobListing(j));
    const healthcareJobs = (healthcareSnap?.exists ? healthcareSnap.data().jobs || [] : []).filter(j => !isInvalidJobListing(j));
    const boardNames = [...loadAtsBoardIndex().values()].map((b) => b.name).filter(Boolean);
    const merged = mergeJobFeeds(atsJobs, adzunaJobs, picks, priorityJobs, boardNames, empleosJobs, healthcareJobs);
...
export async function getJobsByExperience(levels) {
  ...
  const jobs = await getAllJobs();
  return jobs.filter((j) => {
    const level = inferExperienceLevel(j.title, j.description, j);
...
export async function getFresherJobs(levels) {
  ...
  return collapseDuplicatePostings(eligible)
    .map((job) => ({ ...job, level: inferExperienceLevel(job.title, job.description, job) }))
    .sort((a, b) => {
...
/**
 * Live market pulse for the hiring-trends story: collapsed distinct roles,
 * experience mix, role demand, freshness, and hiring concentration.
 */
export async function getJobMarketPulse() {
  const raw = await getAllJobs();
  const jobs = collapseDuplicatePostings(raw);
  const now = Date.now();
  const day = 86_400_000;

  const enriched = jobs.map((j) => {
    const at = Date.parse(j.postedAt || j.firstSeenAt || "") || 0;
    return {
      ...j,
      level: inferExperienceLevel(j.title, j.description, j) || "unknown",
      role: roleFacetKey(j.title) || "Other",
      ageDays: at ? (now - at) / day : null,
  ...
  const byLevel = Object.fromEntries(tally(enriched, (j) => j.level).map(({ name, count }) => [name, count]));
  const known = enriched.filter((j) => j.level !== "unknown");
  const knownLevel = Object.fromEntries(
    tally(known, (j) => j.level).map(({ name, count }) => [
      name,
      { count, pct: known.length ? Math.round((1000 * count) / known.length) / 10 : 0 },
    ])
  );

  const recent7 = enriched.filter((j) => j.ageDays != null && j.ageDays >= 0 && j.ageDays <= 7);
  const recent30 = enriched.filter((j) => j.ageDays != null && j.ageDays >= 0 && j.ageDays <= 30);
  const early = enriched.filter((j) => j.level === "intern" || j.level === "junior");
  const eng = enriched.filter((j) => j.role === "Engineering");
  const engKnown = eng.filter((j) => j.level !== "unknown");
  const engSeniorish = engKnown.filter((j) =>
    ["senior", "lead", "manager"].includes(j.level)
  ).length;

  const byCompany = tally(enriched, (j) => j.company);
  const top10 = byCompany.slice(0, 10);
  const top10Share = top10.reduce((s, c) => s + c.count, 0);
  const multiHire = byCompany.filter((c) => c.count >= 5).length;

  const corridor = enriched.filter((j) =>
    /gachibowli|madhapur|financial district|hitec|raidurg/i.test(
      `${j.area || ""} ${j.location || ""}`
    )
  ).length;

  const dated = enriched.filter((j) => j.ageDays != null && j.ageDays >= 0 && j.ageDays <= 3650);
  const freshness = { today: 0, week: 0, month: 0, older: 0 };
  for (const j of dated) {
    if (j.ageDays <= 1) freshness.today += 1;
    else if (j.ageDays <= 7) freshness.week += 1;
    else if (j.ageDays <= 30) freshness.month += 1;
    else freshness.older += 1;
  }

  return {
    asOf: new Date().toISOString().slice(0, 10),
    totalRoles: enriched.length,
    uniqueEmployers: byCompany.length,
    byCategory: Object.fromEntries(tally(enriched, (j) => j.cat).map(({ name, count }) => [name, count])),
    byRole: tally(enriched, (j) => j.role).slice(0, 12),
    byLevel,
    knownLevel,
    knownCount: known.length,
    recent7: recent7.length,
    recent30: recent30.length,
    recent7Roles: tally(recent7, (j) => j.role).slice(0, 8),
    recent7Levels: Object.fromEntries(tally(recent7, (j) => j.level).map(({ name, count }) => [name, count])),
    earlyCareer: early.length,
    earlyByCompany: tally(early, (j) => j.company).slice(0, 12),
    earlyByCategory: Object.fromEntries(tally(early, (j) => j.cat).map(({ name, count }) => [name, count])),
    engineering: eng.length,
    engSeniorishPct: engKnown.length ? Math.round((1000 * engSeniorish) / engKnown.length) / 10 : 0,
    topCompanies: top10,
    multiHire,
    top10Share,
    top10Pct: enriched.length ? Math.round((1000 * top10Share) / enriched.length) / 10 : 0,
    corridor,
    corridorPct: enriched.length ? Math.round((1000 * corridor) / enriched.length) / 10 : 0,
    freshness,
