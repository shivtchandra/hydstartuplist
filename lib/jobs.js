...
import { cleanScrapedTitle } from "./job-content.js";
...
/**
 * Company + title flattened for duplicate grouping.
 *
 * Drops "and" so the same Wells Fargo role posted as both "Fraud & Claims" and
 * "Fraud and claims" collapses into one row.
 */
function fresherDedupeKey(job) {
  const title = String(job.title || "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  return `${String(job.company || "").toLowerCase()}|${title}`;
}

/**
 * The /jobs/fresher feed: entry-level rows only, one row per distinct opening,
 * with other-metro postings and staffing funnels removed.
 *
...
export async function getFresherJobs(levels) {
  ...
  for (const job of jobs) {
    if (matchesNameList(companyNeedle(job), FRESHER_EMPLOYER_DENYLIST)) continue;
    // A Hyderabad-filtered board can still return a role whose title names another city.
    if (OTHER_METRO_RE.test(job.title || "")) continue;

    const key = fresherDedupeKey(job);
    const existing = grouped.get(key);
    if (!existing) {
      grouped.set(key, { ...job, level: inferExperienceLevel(job.title, job.description), openings: 1 });
      continue;
    }
    existing.openings += 1;
    // Keep the freshest posting as the row the user clicks through to.
    const at = Date.parse(job.postedAt || job.firstSeenAt || "") || 0;
    const best = Date.parse(existing.postedAt || existing.firstSeenAt || "") || 0;
    if (at > best) grouped.set(key, { ...job, level: existing.level, openings: existing.openings });
  }
