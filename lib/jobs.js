...
import { encodeJobsSnapshot, decodeJobsSnapshot, companyResolutions } from "./jobs-cache-codec.js";
...
const getPersistedRuntimeJobs = unstable_cache(
  async () => {
    ...
    return encodeJobsSnapshot({ jobs, sourceStale: jobs.sourceStale === true, resolutions: jobs.companyResolutions });
  },
  ["public-jobs-compressed-v3"],
...
export const getAllJobs = cache(async () => {
  ...
  if (isNextProductionBuild()) {
    ...
    try {
      ...
      console.error("jobs cache capacity exceeded; serving uncached snapshot");
