...
function mergeJobFeeds(atsJobs, adzunaJobs, careerJobs, priorityJobs = [], boardNames = [], empleosJobs = []) {
  ...
  for (const j of empleosJobs) push(j, 2);
  for (const j of careerJobs) push(j, 3);
  for (const j of adzunaFiltered) push(j, 4);
  return out;
}
...
async function loadAllJobs() {
  ...
  try {
    const [atsSnap, adzunaSnap, prioritySnap, empleosSnap] = await Promise.all([
      readFeed("ats_latest"),
    ...
    const merged = mergeJobFeeds(atsJobs, adzunaJobs, picks, priorityJobs, boardNames, empleosJobs);
