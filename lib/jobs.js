...
async function withBoost(jobs) {
  ...
  return jobs
    ...
    .sort(
      (a, b) =>
        new Date(b.sourcePostedAt || b.postedAt || b.firstSeenAt || 0) - new Date(a.sourcePostedAt || a.postedAt || a.firstSeenAt || 0)
    );
}

function mergeJobFeeds(atsJobs, adzunaJobs, careerJobs, priorityJobs = []) {
  // Prefer ATS over Adzuna when apply URLs collide; careers/priority fill gaps.
  const byUrl = new Map();
  const out = [];
  ...
  for (const j of adzunaJobs) push(j, 3);
  return out;
}
...
async function loadAllJobs() {
  ...
  try {
    ...
    const merged = mergeJobFeeds(atsJobs, adzunaJobs, picks, priorityJobs);
