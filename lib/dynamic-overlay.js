...
/** Drop undefined recursively — Firestore rejects undefined field values. */
function omitUndefined(value) {
  if (value === undefined) return undefined;
  if (value === null || typeof value !== "object") return value;
  if (Array.isArray(value)) {
    return value.map(omitUndefined).filter((v) => v !== undefined);
  }
  const out = {};
...
function slimSalary(salary) {
  if (!salary || typeof salary !== "object") return null;
  const out = {};
  if (salary.min != null && Number.isFinite(Number(salary.min))) out.min = Number(salary.min);
  if (salary.max != null && Number.isFinite(Number(salary.max))) out.max = Number(salary.max);
  if (salary.currency) out.currency = String(salary.currency);
...
function slimHiring(hiring) {
  ...
  const roles = Array.isArray(hiring.roles)
    ? hiring.roles.slice(0, 12).map((r) => {
        ...
        const salary = slimSalary(r?.salary);
 ...
 */
/** Normalize rollup doc → [id, slim][] for Map(). */
export function pairsFromRollupData(data) {
  if (!data || typeof data !== "object") return null;
  // Preferred: map field (Firestore forbids nested arrays like [[id, doc], ...])
  if (data.byId && typeof data.byId === "object" && !Array.isArray(data.byId)) {
    return Object.entries(data.byId);
  }
  if (!Array.isArray(data.pairs)) return null;
  const out = [];
  for (const entry of data.pairs) {
    if (Array.isArray(entry) && entry.length >= 2) out.push([String(entry[0]), entry[1]]);
    else if (entry && typeof entry === "object" && entry.id != null) {
      out.push([String(entry.id), entry.data ?? entry.slim ?? entry]);
    }
  }
  return out;
}
...
export async function rebuildDynamicOverlayRollup(db) {
  ...
  snap.forEach((doc) => {
    ...
    if (slim) byId[doc.id] = slim;
  });

  const count = Object.keys(byId).length;
  // Plain JSON only — strips undefined and avoids nested-array pairs tuples.
  ...
  if (approx > 900_000) {
    ...
    return { ok: false, reason: "too-large", bytes: approx, count };
  }

  await rollupRef(db).set(payload);
  return { ok: true, count, bytes: approx };
}
...
export async function fetchOverlayPairsFromFirestore() {
  ...
  if (rollupSnap?.exists) {
    const pairs = pairsFromRollupData(rollupSnap.data());
