...
// Clearbit (legacy) → Google favicon across host variants → DuckDuckGo →
// same-origin favicon paths (helps when Google hasn't indexed the icon yet).
export function logoSrcs(website) {
  const bare = domainOf(website);
  if (!bare) return [];
  const hosts = faviconHosts(website);
  const srcs = [`https://logo.clearbit.com/${bare}?size=128`];
  for (const h of hosts) {
    srcs.push(`https://www.google.com/s2/favicons?domain=${encodeURIComponent(h)}&sz=64`);
  }
  for (const h of hosts) {
    srcs.push(`https://icons.duckduckgo.com/ip3/${encodeURIComponent(h)}.ico`);
  }
  try {
