...
export async function getText(url, timeoutMs = 6000) {
  try {
 ...
 */
export async function fetchBoardJobs(provider, slug, { companyName = null, geoFilter = true } = {}) {
  const meta = boardMeta(provider, slug);
  if (!meta) return { ok: false, jobs: [], boardUrl: null };

  const data = await getJson(meta.apiUrl);
...
export async function probeBoard(provider, slug) {
  const meta = boardMeta(provider, slug);
  if (!meta) return null;
  const data = await getJson(meta.apiUrl, 5000);
  const raw = extractRawJobs(provider, data);
...
export async function classifyBySlugProbes(website, { preferHyd = true } = {}) {
  ...
  for (const slug of slugs) {
    for (const provider of ATS_PROVIDERS) {
      tasks.push(
        (async () => {
          const result = await fetchBoardJobs(provider, slug, { geoFilter: false });
          if (!result.ok || !result.totalRaw) return null;
          const hydJobs = result.jobs.filter((j) => isHydOrTelanganaLocation(j.location));
          return {
            provider,
            slug,
            boardUrl: result.boardUrl,
            jobCount: result.totalRaw,
            hydCount: hydJobs.length,
          };
        })()
      );
    }
  ...
  const hits = (await Promise.all(tasks)).filter(Boolean);
  if (!hits.length) return null;
  hits.sort((a, b) => {
    if (preferHyd && b.hydCount !== a.hydCount) return b.hydCount - a.hydCount;
    return b.jobCount - a.jobCount;
  });
  // Prefer a board with Hyd roles; otherwise still classify (sync will store 0 Hyd jobs)
  const best = preferHyd ? hits.find((h) => h.hydCount > 0) || hits[0] : hits[0];
  return best;
}
