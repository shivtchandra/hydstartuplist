...
export async function fetchBoardJobs(provider, slug, { companyName = null, geoFilter = true, withContent = true } = {}) {
  ...
  if (provider === "radancy") {
    ...
    raw = extractRawJobs(provider, data) || [];
  }

  if (!Array.isArray(raw) || raw.length === 0) {
    return { ok: false, jobs: [], boardUrl: meta.boardUrl, totalRaw: 0 };
  }
...
async function fetchEightfoldPaged(meta) {
  ...
  while (start < 500 && out.length < total) {
    ...
    out.push(...batch);
    total = data?.data?.count || data?.count || out.length;
...
async function fetchGr8PeoplePaged(meta) {
  ...
  while (start < total && start < 500) {
    try {
      ...
      out.push(...batch);
      total = typeof results?.totalCount === "number" ? results.totalCount : out.length;
      if (!batch.length) break;
      start += batch.length;
    } catch {
      break;
...
async function fetchSuccessFactorsJobs(meta) {
  ...
  while (pageNumber < 40 && out.length < total) {
    try {
      ...
      out.push(...batch);
      total = typeof data.totalJobs === "number" ? data.totalJobs : out.length;
      if (!batch.length) break;
      pageNumber += 1;
    } catch {
      break;
    ...
    for (const m of html.matchAll(/href=["']((?:https?:\/\/[^"'\s]+)?\/jobs\/[^"'#?]+\/?)["']/gi)) {
      ...
      try {
        ...
        if (jr.ok) {
          ...
          if (locM) location = String(locM[1] || locM[0] || "").replace(/\s+/g, " ").trim();
