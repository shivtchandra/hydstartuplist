...
const DESC_MAX = 1200; // short blurbs — ats_latest is packed leaner in sync-ats-jobs
...
function descriptionOf(provider, job) {
  ...
  else if (provider === "jobsyn") raw = job.description || null;
  else if (provider === "rippling") raw = job.description || job.jobDescription || null;
  else if (provider === "freshteam") raw = job.description || job.job_description || job.plain_description || null;
  if (!raw) return null;
...
function externalIdOf(provider, job) {
  ...
  if (provider === "phenom") {
    ...
    return String(j.req_id || j.reqId || j.jobSeqNo || j.slug || j.id || "");
  }
  ...
  if (provider === "jobsyn") return String(job.reqid || job.guid || job.id || "");
  if (provider === "rippling") return String(job.uuid || job.id || "");
  if (provider === "freshteam") return String(job.id || job.job_id || "");
  return String(job.id || job.url || "");
}

/** Phenom often returns iCIMS `/login` stubs that bounce to a generic careers wall. */
function isPhenomLoginStub(url) {
  return /icims\.com\/jobs\/[^/]+\/login\/?$/i.test(String(url || ""));
}

function phenomPublicJobUrl(slug, boardUrl, jobUrlBase) {
  if (!slug) return null;
  const base =
    jobUrlBase ||
    (boardUrl && String(boardUrl).includes("/jobs")
      ? String(boardUrl).split("?")[0].replace(/\/page\/\d+$/i, "").replace(/\/?$/, "")
      : null) ||
...
function applyUrlOf(provider, job, boardUrl, { jobUrlBase } = {}) {
  if (provider === "phenom") {
    const j = job.data || job;
    const slug = j.slug || j.req_id || j.reqId || j.jobSeqNo;
    const candidates = [j.applyUrl, j.apply_url, j.canonicalPositionUrl, j.url];
    for (const c of candidates) {
      if (c && !isPhenomLoginStub(c)) return c;
    }
    return phenomPublicJobUrl(slug, boardUrl, jobUrlBase) || boardUrl;
  }
  if (provider === "eightfold") {
    const path =
      job.canonicalPositionUrl ||
      job.positionUrl ||
      (job.id ? `/careers/job/${job.id}` : null);
    if (path) {
  ...
  if (provider === "jobsyn") {
    const reqid = job.reqid || job.id;
    if (reqid && jobUrlBase) {
      return `${jobUrlBase.replace(/\/?$/, "")}?jobId=${encodeURIComponent(String(reqid))}&source=JB_DirectEmployers`;
    }
    if (job.guid) return `https://rr.jobsyn.org/${job.guid}`;
  }
  ...
  if (provider === "freshteam") {
    return job.url || job.hosted_url || (job.id && jobUrlBase ? `${jobUrlBase.replace(/\/?$/, "")}/${job.id}` : boardUrl);
  }
  if (provider === "rippling") {
    return job.url || (job.uuid && jobUrlBase ? `${jobUrlBase.replace(/\/?$/, "")}/${job.uuid}` : boardUrl);
  }
  return (
    job.absolute_url ||
    job.hostedUrl ||
...
export function toPublicJob(
  ...
  return {
    ...
    sourcePostedAt: normalized.postedAt || null,
    firstSeenAt: fetchedAt || new Date().toISOString(),
    lastSeenAt: fetchedAt || new Date().toISOString(),
    lastCheckedAt: fetchedAt || new Date().toISOString(),
    status: "active",
