import { sanitizeJobHtml, normalizeSalary } from "../job-content.js";
...
function descriptionOf(provider, job) {
  ...
  const sanitized = sanitizeJobHtml(asString);
  if (!sanitized) return null;
  return sanitized.length > DESC_MAX ? sanitized.slice(0, DESC_MAX) : sanitized;
}
...
export function toPublicJob(normalized, { startupId = null, company, fetchedAt } = {}) {
  ...
  return {
    ...
    description: normalized.descriptionHtml || null,
