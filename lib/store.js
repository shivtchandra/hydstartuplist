...
// React.cache is ESM-named in Next RSC; plain Node (GH Actions) sees CJS React.
...
const OVERLAY_TTL_MS = 5 * 60_000; // was 60s — full collection get is expensive
...
async function getDynamicOverlay() {
  ...
  // Never block `next build` on Firestore — Hobby SSG workers SIGTERM at ~60–180s.
...
/** After cron/admin writes to startups_dynamic — refresh rollup + caches. */
...
export const getApproved = cache(async () => {
