...
export function cleanJobDescriptionHtml(raw) {
  if (!raw || typeof raw !== "string") return "";
  let text = raw.trim();
  // Unescape double-escaped HTML entities if present
  if (/&lt;\/?(p|div|ul|li|h[1-6]|span|b|strong|br|em|i)\b/i.test(text)) {
    text = text
      .replace(/&lt;/g, "<")
      .replace(/&gt;/g, ">")
      .replace(/&quot;/g, '"')
  ...
  // If plain text with newlines and no HTML tags, convert newlines to paragraphs
...
export const ADZUNA_FRESH_DAYS = 1.5;
...
export const NON_JOB_URL_RE =
  /\/(?:speciality|specialties|specialty|treatments?|procedures?|doctors?|departments?|services?|packages?|health-package|blogs?|news|articles?|patient-care|about-us|contact-us|facilities|infrastructure|conditions|clinics?|children-care-hospital|care-hospital|collections\/|products\/|category\/|shop\/)\b/i;

export const NON_JOB_TITLE_RE =
  /\b(?:treatment|treatments|procedure|procedures|speciality|specialties|specialty|department|doctor profile|patient care|book appointment|consult doctor|health checkup|opd|emergency service|our doctors|find a doctor|health tips|medical service|hospital in\b|best hospital|care hospital|view open|submit resume|helpdesk|hr suite|hr templates|hr analytics)\b/i;

export function isInvalidJobListing(job) {
  if (!job) return true;
  const url = String(job.url || job.applyUrl || "");
  const title = String(job.title || "").trim();
  if (!title || title.length < 3) return true;
  if (url && NON_JOB_URL_RE.test(url)) return true;
  if (NON_JOB_TITLE_RE.test(title)) return true;
  return false;
}

/**
 * Filter out third-party staffing consultancies, body-shop listings,
 * non-job URLs/pages, and vague ghost-employer posts from aggregators.
 */
export function isSpamOrConsultancyJob(job) {
  if (!job) return true;
  if (isInvalidJobListing(job)) return true;
