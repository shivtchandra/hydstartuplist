  ...
  for (let i = 0; i < pageJobItems.length; i += BATCH_SIZE) {
    ...
    await Promise.all(
      chunk.map(async (item) => {
        ...
        const slugCityMatch = item.route.match(/-jobs-in-([a-z]+(?:-[a-z]+)*)/i);
        const slugCity = slugCityMatch
          ? slugCityMatch[1].replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())
          : null;
        ...
        if (DIETETICS_ROLE_RE.test(slugTitle)) {
          jobs.push({
            ...
            location: slugCity || "Hyderabad, Telangana",
            url: item.url,
...
export async function fetchSwaasaDietitianJobs() {
  ...
  const urls = [
    ...
    "https://www.swaasa.com/in/en/c/dietitiannutritionist-jobs?from=20&s=1",
  ];

  for (const kw of searchKeywords) {
    urls.push(`https://www.swaasa.com/in/en/search-results?keywords=${encodeURIComponent(kw)}&from=0&s=1`);
    urls.push(`https://www.swaasa.com/in/en/search-results?keywords=${encodeURIComponent(kw)}&from=10&s=1`);
  }
  ...
  for (const url of urls) {
    try {
      ...
      for (const r of rawJobs) {
        ...
        if (!DIETETICS_ROLE_RE.test(title) && !DIETETICS_ROLE_RE.test(r.title || "") && !DIETETICS_ROLE_RE.test(r.category || "")) {
          continue;
        }
