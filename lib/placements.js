...
let grantedSlotsCache = null;
let grantedSlotsFetchedAt = 0;
const GRANTED_SLOTS_TTL_MS = 5 * 60_000;

async function getGrantedSlots(now = Date.now()) {
  if (isNextProductionBuild()) return [];
  if (grantedSlotsCache && Date.now() - grantedSlotsFetchedAt < GRANTED_SLOTS_TTL_MS) {
    return grantedSlotsCache.filter((s) => inWindow(s, now));
  }
  const db = await getAdminDb();
  if (!db) return [];
  try {
    const snap = await withTimeout(db.collection("granted_placements").get(), 5_000, null);
    if (!snap) return grantedSlotsCache || [];
    ...
    if (requestIds.length) {
      ...
      await Promise.all(requestIds.map(async (requestId) => {
        const req = await withTimeout(db.collection("featured_requests").doc(requestId).get(), 3000, null);
    ...
    grantedSlotsCache = filtered;
    grantedSlotsFetchedAt = Date.now();
    return filtered.filter((s) => inWindow(s, now));
  } catch {
    return (grantedSlotsCache || []).filter((s) => inWindow(s, now));
  }
}
...
function boostMatches(job, match = {}) {
...
export function jobIsBoosted(job, boosts = getJobBoosts()) {
  ...
  return boosts.some(({ match }) => boostMatches(job, match));
}

/** Matching boost slot for a job, if any. */
export function jobBoostFor(job, boosts = getJobBoosts()) {
  if (!boosts.length || !job) return null;
  return boosts.find(({ match }) => boostMatches(job, match)) || null;
}
