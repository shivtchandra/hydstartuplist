...
const FRESHER_RE =
  /\bintern\b|\binterns\b|\binternship\b|fresher|trainee|campus hire|new[\s-]*grad|entry[\s-]*level|graduate engineer|graduate trainee/;
...
export function titleSeniority(title) {
  ...
  if (/\b(vp|vice president|director|head of|general manager|gm|chief|cxo|ceo|cto|cfo|coo|avp)\b/.test(t)) {
    return "manager";
  }
  ...
  if (
    /\bmanager\b/.test(t) &&
    !/\b(product manager|project manager|account manager|office manager|success manager|community manager|brand manager|channel manager)\b/.test(
      t
    )
  ) {
    return "manager";
  ...
  // A level suffix of II or higher is never an entry band ("Analyst II", "Engineer III").
  if (/(?:^|[\s,\-(/])(?:ii|iii|iv)(?=$|[\s,\-)./])/.test(t)) return "mid";
...
export function inferExperienceLevel(title, description) {
  ...
  // An explicit fresher word in the title outranks everything else.
  if (FRESHER_RE.test(titleText)) return "intern";

  // A senior band in the title wins over any years/intern wording in the body.
