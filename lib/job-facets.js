...
export function inferExperienceLevel(title, description, job) {
  ...
  if (jobObj?.level && EXPERIENCE_OPTIONS.some((o) => o.key === jobObj.level)) {
    return jobObj.level;
  }

  // Explicit startYear / endYear structured fields (e.g. Docthub)
  if (jobObj && Number.isFinite(Number(jobObj.startYear))) {
    ...
    if (lo === 0 && hi <= 1) return "intern";
    if (hi <= 2 && lo <= 2) return "junior";
    if (lo >= 8 || hi >= 12) return "lead";
    if (lo >= 5) return "senior";
    if (lo >= 2 || hi >= 3) return "mid";
  }
  ...
  // A fresher word in the body also outranks a years-of-experience requirement mentioned
  // elsewhere in the description (e.g. "5 years in business" in a company-history blurb).
  if (FRESHER_RE.test(head) || /\b0[\s-]*1\s*years?\b/.test(t)) return "intern";

  // Explicit high-experience requirements (e.g. "5+ years of experience", "3-5 years exp").
  // Requires the word experience/exp nearby so a company-age or benefits mention
  // ("founded 8 years ago", "3 years of paid leave") doesn't falsely demote a fresher posting.
  const highExpMatch = descText.match(
    /\b([3-9]|\d{2,})\s*\+?\s*[-–to]?\s*(\d+)?\s*\+?\s*years?\s*(?:of\s*)?(?:relevant\s*|prior\s*|professional\s*|work\s*)?(?:experience|exp\.?)\b/
  );
  if (highExpMatch) {
...
export function jobExperienceDisplay(job) {
  ...
  if (job.experience && typeof job.experience === "string") {
    ...
    const m = exp.match(/(\d+)\s*(?:[-–—]|to)\s*(\d+)\s*\+?\s*(?:years?|yrs?)/i);
    if (m) return `${m[1]}–${m[2]} yrs`;
    const pm = exp.match(/(\d+)\s*\+\s*(?:years?|yrs?)/i);
    if (pm) return `${pm[1]}+ yrs`;
  ...
  // 3. Search title & description for exact years
  const text = `${job.title || ""} ${job.description || ""}`;
  const rangeMatch = text.match(/\b(\d+)\s*(?:[-–—]|to)\s*(\d+)\s*\+?\s*(?:years?|yrs?)(?:\s+of\s+experience)?\b/i);
  if (rangeMatch) {
  ...
  const plusMatch = text.match(/\b(\d+)\s*\+\s*(?:years?|yrs?)(?:\s+of\s+experience)?\b/i);
  if (plusMatch) {
    const yrs = Number(plusMatch[1]);
