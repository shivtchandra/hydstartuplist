...
async function getDynamicOverlay({ fresh = false } = {}) {
  if (!fresh && dynamicOverlayCache && Date.now() - dynamicOverlayFetchedAt < OVERLAY_TTL_MS) {
  ...
  try {
    ...
    let pairs = null;
    if (fresh) {
      // Admin / post-approve paths must not read the 5-minute Next data cache.
      ...
      try {
        ...
        const cached = unstable_cache(
          ...
          ["startups-dynamic-overlay-v3-byId"],
...
export async function getApproved({ fresh = false } = {}) {
  const base = readJson(APPROVED_FILE);
  const overlay = await getDynamicOverlay({ fresh });
