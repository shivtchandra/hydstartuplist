...
import { prepareJobs, filterJobs, mapGroups, jobSummary, readFilters, capEmployerShare, isHydArea } from './opportunities.js';
...
export async function searchOpportunities(filters={},cursor='') {
  ...
  const matched=filterJobs(data.jobs,filters);
  const jobs=matched;
  ...
  const hydAreas=[...new Set(active.map(j=>j.area).filter(a=>a&&isHydArea(a)))].sort();
  return {jobs:jobs.slice(offset,next).map(jobSummary),total:jobs.length,employers:mapGroups(matched).length,
    ...
    facets:{roles:unique('role'),areas:hydAreas,levels:unique('level'),types:unique('category')},
