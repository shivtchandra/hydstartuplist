...
export async function GET(req) {
  try {const data=await opportunityDataset();const jobs=filterJobs(data.jobs,readFilters(new URL(req.url).searchParams));
    return NextResponse.json({companies:mapGroups(jobs),total:jobs.length,version:data.version,stale:data.stale,work:jobs.reduce((out,j)=>({...out,[j.work]:(out[j.work]||0)+1}),{})});
