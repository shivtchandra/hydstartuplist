...
export async function GET(req) {
  ...
  const [boards, usage, sessions] = await Promise.all([
  ...
  const funnel = {};
  const byProduct = { startups: { landings: 0, useful: 0, apply: 0, sessions: 0 }, eateries: { landings: 0, useful: 0, apply: 0, sessions: 0 } };

  for (const doc of sessions.docs) {
    const s = doc.data();
    if (s.events?.landing === undefined) continue;
    const product = s.product === 'eateries' ? 'eateries' : 'startups';
