    ...
    <div className="page-with-nav">
      ...
      <article className="story-page">
        ...
        <div className="story-body">
          ...
          <ol>
            {COMPANIES.map((c) => (
              <li key={c.slug} style={{ marginBottom: 14 }}>
                <strong>
                  <Link href={`/startups/${c.slug}`}>{c.name}</Link>
                </strong>{" "}
                <span style={{ opacity: 0.75 }}>({c.angle} · {c.area})</span>
                <br />
                {c.line}{" "}
                <Link href={`/startups/${c.slug}`}>View on the map →</Link>
              </li>
            ))}
          </ol>
          ...
          <p>
            <Link href="/startups/skyroot-aerospace">Skyroot Aerospace</Link>,
          ...
          <p>
            <Link href="/startups/dhruva-space">Dhruva Space</Link> in
          ...
          <p>
            <Link href="/startups/cosmoserve-space">Cosmoserve Space</Link> is a
          ...
          <p>
            ...
            <Link href="/startups/jeh-aerospace">JEH Aerospace</Link> does
            precision aero/defense manufacturing;{" "}
            <Link href="/startups/raghu-vamsi-aerospace-group">
              Raghu Vamsi Aerospace Group
            </Link>{" "}
            in Kukatpally is a larger manufacturing group (including a widely reported ~$40M raise);
            and{" "}
            <Link href="/startups/apollo-micro-systems">Apollo Micro Systems</Link>{" "}
          ...
          <ul>
            {ADJACENT.map((c) => (
              <li key={c.slug}>
                <Link href={`/startups/${c.slug}`}>{c.name}</Link> ({c.area}) — {c.line}
              </li>
            ))}
          </ul>
          <p>
            <Link href="/startups/indrajaal">Indrajaal</Link> has been
            among the more active hirers in our careers sync;{" "}
            <Link href="/startups/marut-drones">MARUT Drones</Link> spans
